#!/usr/bin/env python3
"""
Requirements: pip install curl_cffi pyotp

Usage:
  python3 create_session_curl.py <用户名> <密码> [TOTP密钥] [--append ../sessions.jsonl]

Examples:
  python3 create_session_curl.py myusername mypassword TOTP_SECRET --append ../sessions.jsonl

Output: 一行 JSON，或追加到指定文件。速度比浏览器方式快，但更容易触发风控。
"""
...
try:
    ...
    print("请先安装: pip install curl_cffi", file=sys.stderr)
    sys.exit(1)
...
def make_request(session, headers, flow_token, subtask_data, print_msg):
    ...
    if not new_flow_token:
        raise Exception(f"步骤失败: {print_msg}")
    return new_flow_token, data


def get_guest_token(session):
    print("[*] 获取 guest token...", file=sys.stderr)
    ...
    if not guest_token:
        raise Exception("无法获取 guest token")
    return guest_token


def init_flow(session, guest_token):
    print("[*] 初始化登录流程...", file=sys.stderr)
    ...
    if not flow_token:
        raise Exception("无法获取 flow token")
...
def submit_username(session, flow_token, headers, guest_token, username):
    ...
    flow_token, data = make_request(session, headers, flow_token, subtask, "提交用户名")
    if data.get("subtasks") and "cta" in data["subtasks"][0]:
        error_msg = data["subtasks"][0]["cta"].get("primary_text", {}).get("text")
        if error_msg:
            raise Exception(f"登录被拒绝: {error_msg}")
    return flow_token
...
def submit_2fa(session, flow_token, headers, guest_token, totp_seed):
    ...
    flow_token, _ = make_request(session, headers, flow_token, subtask, "提交 2FA")
    return flow_token
...
def submit_js_instrumentation(session, flow_token, headers, guest_token):
    ...
    flow_token, _ = make_request(session, headers, flow_token, subtask, "JS instrumentation")
    return flow_token
...
def complete_flow(session, flow_token, headers):
    ...
    make_request(session, headers, flow_token, subtask, "完成登录")
...
def main():
    if len(sys.argv) < 3:
        print("用法: python3 create_session_curl.py <用户名> <密码> [TOTP密钥] [--append sessions.jsonl]", file=sys.stderr)
        sys.exit(1)
